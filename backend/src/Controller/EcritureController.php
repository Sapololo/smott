...
class EcritureController extends ApiController {
  ...
  /**
   * @Rest\View()
   * @Rest\Get("/api/ecrituresAdherent/{licenceId}")
   */
  public function ecrituresAdherent(EcritureRepository $ecritureRepository, int $licenceId) {
    $ecritures = $ecritureRepository->findByAdherent($licenceId);
    return $this->respond($ecritures);
  }
...
//    print_r($data->getType()->getId());
    $ecriture->setEcrrubriqueId($data->getType()->getId());
...
//    print_r($data->getRubrique()->getId());
    if ($data->getAdherent() !== null) {
      $ecriture->setLicenceId($data->$data->getAdherent()->getId);
    }
//    print_r($ecriture);
//    die('coucou');
    $em->persist($ecriture);
   ...
   * @throws \Doctrine\ORM\NonUniqueResultException
   */
  public function updateEcriture(EcritureDto $data, EntityManagerInterface $em) {

    /** @var EcritureRepository $ecr */
    $ecr = $em->getRepository(EcritureId::class);
...
//    $ecriture = $ecr->find($id);
    if (!$ecriture) {
      ...
      $ecriture->setPaiementId($data->getType()->getId());
