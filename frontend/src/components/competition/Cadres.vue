      ...
      <v-layout column>
        <v-container fluid grid-list-md>
          <v-layout row wrap v-if="loading">
            <v-flex md4 sm6 v-for="(item, i) in listeJoueurs" :key="i" v-if="item.tech">
              <v-card color="primary">
                <v-card-text>
                  <h1 class="headline white--text" v-html="item.nom +' '+ item.prenom"/>
                  <p class="back--text" v-html="item.tech"/>
                </v-card-text>
              </v-card>
            </v-flex>
          </v-layout>
...
export default class Cadres extends Vue {
  @GetterDrawer
  ...
  async mounted () {
    ...
    forEach(this.listeJoueurs, (joueur, key) => {
      if (joueur.tech === '' && this.listeJoueurs !== null) {
